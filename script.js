...
function live() {
    ...
    const getCurrentBox = () => {
        let firstFound = null;
        boxes.forEach((box) => {
            if (box.textContent === "" && firstFound === null) {
                firstFound = box;
            }
        });
        return [firstFound];
    ...
    const flipTile = (tile, timeout) => {
        let timeA = setTimeout(() => {
            tile.classList.add("animate");
            clearTimeout(timeA);
        }, timeout);
        let timeB = setTimeout(() => {
            tile.classList.remove("animate");
            clearTimeout(timeB);
        }, 500 + timeout);
    };

    const shakeTiles = (row) => {
        for (let i = 0; i < row.length; i++) {
            const tile = row[i];
    ...
    const colorKeyBoardKey = (letter, color) => {
        const keys = [...document.querySelectorAll(".key")];
        let key;
        keys.forEach((k) => {
            if (k.textContent === letter) key = k;
        });
    ...
    const colorCode = (guessParam) => {
        let greenLetters = [];
        let yellowLetters = [];
        const currentRow = getCurrentRow().children;
        let timer = 0;
        for (let i = 0; i < 5; i++) {
            const letter = guessParam[i];
            const compWord = word.toUpperCase()[i];
            if (letter === compWord) {
                ...
                timer += 250;
                colorKeyBoardKey(letter, "gray");
            }
        }
    };

    const winBounce = (tile, timeout) => {
        let timeA = setTimeout(() => {
    ...
    const presskey = (lowerKey) => {
        ...
        if (isLetter(key)) {
            if (guess.split("").length < 5) {
                const currentBox = getCurrentBox()[0];
                if (currentBox && !won) {
            ...
            if (key === "BACKSPACE") {
                //Backspace Character
                const previousBox = getCurrentBoxForDeleting();
                if (previousBox !== null) {
                ...
                if (guess.split("").length === 5) {
                    ...
                    if (dictionary.includes(guess.toLowerCase())) {
                        ...
                        if (guess === word) {
                            ...
                            for (let i = 0; i < 5; i++) {
                                let a = setTimeout(() => {
                                    winBounce(row.children[i], timeout);
                                    timeout += 500;
                                    clearTimeout(a);
                                }, 250 * 5);
                            }
